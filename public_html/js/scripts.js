$(document).ready(function () {
	...
	// 	$.pjax.reload({container: "#pj-disciplines", url: '".\yii\helpers\Url::to(['discipline/view_location'])."', data: {id: $(this).val()}});
	// });

	$('[data-toggle="popover"]').popover({html:true}); 

	$('#program-input').on('blur', function () {
    var $yiiform = $('#form-program');
    ...
    $.ajax({
            ...
            success: function( data, status, xhttp) {
             // data will be true or false if you returned a json bool
                ...
                // }, 1500);
                if(data.success) {
                  $('#program-success').addClass('show');
                 
                } else {
                	$('#program-error').addClass('show');
                }
            },
        }
    )
