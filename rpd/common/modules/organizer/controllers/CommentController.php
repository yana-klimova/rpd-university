    ...
    public function actionCreate($id_discipline)
    {
        ...
        if(Yii::$app->request->isPjax){
            ...
            if($commentModel->save()) {
                $commentModel = new Comment();
                return $this->renderAjax('/discipline/_comment', compact('commentSearchModel', 'commentDataProvider', 'discipline', 'tabs', 'commentModel')); 
            } else {
                return $this->renderAjax('/discipline/_comment', compact('commentSearchModel', 'commentDataProvider', 'discipline', 'tabs', 'commentModel'));
            }
        } else {
            return $this->redirect(['/discipline/view', 'id'=>$discipline->id, '#'=>'comment']);
        }
    }
    ...
    public function actionUpdate($id_comment, $id_discipline)
    {
        ...
        if($commentModel->load(Yii::$app->request->post()) && $commentModel->save()){
           return $this->redirect(['discipline/view', 'id'=>$discipline->id, '#'=>'comment']);
            ...
            return $this->render('update', compact('commentModel', 'tabs', 'discipline'));
    }
}
